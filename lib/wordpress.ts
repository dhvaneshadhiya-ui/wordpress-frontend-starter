...
const API_BASE = process.env.WORDPRESS_API_URL || process.env.NEXT_PUBLIC_WORDPRESS_API_URL || 'https://dev.igeeksblog.com/wp-json/wp/v2';
...
export async function fetchPosts(params: {
  ...
  const response = await fetch(`${API_BASE}/posts?${searchParams}`, fetchOptions);
